...
# Amostra: 1 mês (31 observações)
#
#
# --------------------------------- Aleatório --------------------------------- #
# Escolher aleatoriamente as curvas até as 148. Escolho 10 quaisquer e agrupo, 
# depois para a quantidade de 20 escolho mais 10 aleatório (diferente dos 10 primeiros) 
# e soma com os outros 10 anterior, somando 20, e assim por diante.
# ----------------------------------------------------------------------------- #
...
for(i in 1:(length(arquivos))){
  ...
  # Diminuindo para 31 observações (1 mês)
  u$date <- substr(u[ ,1],0,10) # Retirando a hora
  u <- aggregate(u[,2], list("date" = u[,1]), FUN = sum)
  #
  if(nrow(u) > 31){
    u <- u[-1,] # Retirando dados de Dezembro
  }
  #
  u[,1] <- as.POSIXct(u[,1], origin="1970-01-01")
  ...
  if( i==1 ){ # No 1º passo recebe "day" e "consumption"
    series <- u
...
# Sequência (aleatória) das curvas de carga em cada execução
curvasAleatorias <- c()
...
# Média
write.csv(silhouetteValues, file = "../Results/silhueta_31_observacoes(30_execucoes).csv")
write.csv(maeValues, file = "../Results/mae_31_observacoes(30_execucoes).csv")
# Mediana
...
write.csv(silhouetteValues, file = "../Results/silhueta_31_observacoes(30_execucoes - mediana).csv")
write.csv(maeValues, file = "../Results/mae_31_observacoes(30_execucoes - mediana).csv")
# -------------------------------------------------------------- #
...
gg1 <- ggplot() +
  ...
  xlab('Qtd de usuários (1 mês)') + ylab('Dunn Index') +
  ggtitle('Comparação das medidas de similaridade') + 
...
# Median :0.06939   Median :0.08035   Median :0.06899
# Mean   :0.07010   Mean   :0.08122   Mean   :0.06883
...
summary(maeValues)
# maeCorr            maeEuclid          maeDtw         
# Median :0.003639   Median :0.003463   Median :0.003933
# Mean   :0.003640   Mean   :0.003463   Mean   :0.003922


# Figura_31Obs_mes_10-148_curvas.png (845x500)
g2 <- ggplotGrob(gg2)
g4 <- ggplotGrob(gg3)
g <- rbind(g2, g4, size="first") # Para as imagens ficar com o mesmo tamanho (alinhadas)
g$widths <- unit.pmax(g2$widths, g4$widths)

png(file="../Results/Images/Figura_31Obs_mes_10-148_curvas.png", width = 845, height = 500)
grid.newpage()
grid.draw(g)
dev.off()
...
# Alternatively we could consider the p-value. With a significance level of 95%, remember this rule: If p-value is 
# greater than 0.05 then we accept the null hypothesis H0; if p-value is less than 0.05 then we reject the null 
# hypothesis H0 in favor of the alternative hypothesis H1.

# Silhouette
t.corr <- t.test(silhouetteValues$silhouetteCorr) # 95 percent confidence interval: 0.06895819 0.07123987
t.dtw <- t.test(silhouetteValues$silhouetteDtw) # 95 percent confidence interval: 0.06737208 0.07029684
t.euclid <- t.test(silhouetteValues$silhouetteEuclid) # 95 percent confidence interval: 0.07940596 0.08302993
...
ggIC_sil <- ggplot(df, aes(x=Medidas, y=Media, group=1)) +
  ...
  ggtitle('IC com 95% (Silhueta - 31 observações)') +
...
# MAE
t.mae.corr <- t.test(maeValues$maeCorr) # 95 percent confidence interval: 0.003622135 0.003657197
t.mae.dtw <- t.test(maeValues$maeDtw) # 95 percent confidence interval: 0.003899802 0.003943796
t.mae.euclid <- t.test(maeValues$maeEuclid) # 95 percent confidence interval: 0.003446216 0.003479124
...
ggIC_mae <- ggplot(df.mae, aes(x=Medidas, y=Media, group=1)) +
  labs(title="IC com 95% (MAE - 31 observações)", x="Medidas", y="MAE") +
  geom_errorbar(width=.2, aes(ymin=IC.Inferior, ymax=IC.Superior)) +
  geom_point(shape=21, size=2, fill="white") +
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5))

# IC_Silhueta_MAE_31_Observacoes.png (850 x 460)
png(file="../Results/Images/IC_Silhueta_MAE_31_Observacoes.png", width = 850, height = 460)
...
# Figura_31Obs_mes_10-148_curvas(IC).png (845x500)
png(file="../Results/Images/Figura_31Obs_mes_10-148_curvas(IC).png", width = 845, height = 500)
grid.arrange(comparacao, ic, ncol = 2, widths=c(4/5,2/5),
             top=textGrob("Comparação das medidas de dissimilaridade (31 observações)", gp=gpar(cex=1.5)))
dev.off()
