...
# Amostra: 1 semana (7 observações, média dos dias do mês Janeiro)
#
...
dayWeek <- c("01","02","03","04","05","06","07")
for(i in 1:(length(arquivos))){
  ...
  for(j in 1:length(semana)){
    u1 <- rbind(u1, mean( u[is.element(weekdays(as.Date(u[,1])), semana[j]), ]$consumption ))
  }
  u1 <- as.data.frame(u1)
  u1$day <- dayWeek
  u1 <- data.frame(day=u1$day, consumption=u1$V1)
  u <- normDataPerfil(u1)
...
# Média
write.csv(silhouetteValues, file = "../Results/silhueta_7_observacoes(30_execucoes).csv")
write.csv(maeValues, file = "../Results/mae_7_observacoes(30_execucoes).csv")
# Mediana
...
write.csv(silhouetteValues, file = "../Results/silhueta_7_observacoes(30_execucoes - mediana).csv")
write.csv(maeValues, file = "../Results/mae_7_observacoes(30_execucoes - mediana).csv")
# -------------------------------------------------------------- #
...
gg1 <- ggplot() +
  ...
  xlab('Qtd de usuários (1 semana)') + ylab('Dunn Index') +
...
# Median :0.2252   Median :0.2239    Median :0.1810
# Mean   :0.2241   Mean   :0.2246    Mean   :0.1830
...
summary(maeValues)
# maeCorr           maeEuclid         maeDtw         
# Median :0.01637   Median :0.01573   Median :0.02127
# Mean   :0.01639   Mean   :0.01576   Mean   :0.02116

# Figura_7Obs_semana_10-148_curvas.png (845x500)
g2 <- ggplotGrob(gg2)
g4 <- ggplotGrob(gg3)
g <- rbind(g2, g4, size="first") # Para as imagens ficarem com o mesmo tamanho (alinhadas)
g$widths <- unit.pmax(g2$widths, g4$widths)

png(file="../Results/Images/Figura_7Obs_semana_10-148_curvas.png", width = 845, height = 500)
grid.newpage()
grid.draw(g)
dev.off()
...
# Silhouette
t.corr <- t.test(silhouetteValues$silhouetteCorr) # 95 percent confidence interval: 0.2200414 0.2282198
t.dtw <- t.test(silhouetteValues$silhouetteDtw) # 95 percent confidence interval: 0.1784232 0.1875733
t.euclid <- t.test(silhouetteValues$silhouetteEuclid) # 95 percent confidence interval: 0.2210109 0.2282198
...
ggIC_sil <- ggplot(df, aes(x=Medidas, y=Media, group=1)) +
  ...
  ggtitle('IC com 95% (Silhueta - 7 observações)') +
...
# MAE
t.mae.corr <- t.test(maeValues$maeCorr) # 95 percent confidence interval: 0.01626304 0.01651533
t.mae.dtw <- t.test(maeValues$maeDtw) # 95 percent confidence interval: 0.02091473 0.02141067
t.mae.euclid <- t.test(maeValues$maeEuclid) # 95 percent confidence interval: 0.01566469 0.01585777
...
ggIC_mae <- ggplot(df.mae, aes(x=Medidas, y=Media, group=1)) +
  labs(title="IC com 95% (MAE - 7 observações)", x="Medidas", y="MAE") +
  geom_errorbar(width=.2, aes(ymin=IC.Inferior, ymax=IC.Superior)) +
  geom_point(shape=21, size=2, fill="white") +
  theme_bw() + theme(plot.title = element_text(hjust = 0.5))

# IC_Silhueta_MAE_7_Observacoes.png (850 x 460)
png(file="../Results/Images/IC_Silhueta_MAE_7_Observacoes.png", width = 850, height = 460)
...
# Figura_7Obs_semana_10-148_curvas(IC).png (845x500)
png(file="../Results/Images/Figura_7Obs_semana_10-148_curvas(IC).png", width = 845, height = 500)
grid.arrange(comparacao, ic, ncol = 2, widths=c(4/5,2/5),
             top=textGrob("Comparação das medidas de dissimilaridade (7 observações)", gp=gpar(cex=1.5)))
dev.off()
