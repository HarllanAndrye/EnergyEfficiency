# --------------- Pacotes necessarios --------------- #
#install.packages('cluster')
library(cluster)
library(fpc)
#install.packages('dtw')
library(dtw)

# Medidas de similaridade entre séries temporais
#install.packages('TSdist')
library(TSdist)

#install.packages("reshape")
#library(reshape)

#install.packages('ggplot2')
require(ggplot2)
#install.packages('gridExtra')
require(gridExtra)
# --------------------------------------------------- #


# Normaliza os dados com valores entre 0 e 1
# (http://www.leg.ufpr.br/lib/exe/fetch.php/projetos:dataminingcomr.pdf)
normData <- function(dados){
  for(i in 1:ncol(dados)){
    if((max(dados[,i]) - min(dados[,i]))!= 0){ # Se e igual a zero, nao precisa normalizar!
      dados[,i] = (dados[,i] - min(dados[,i]))/(max(dados[,i]) - min(dados[,i]))
    }
  }
  return(dados)
}


# Normaliza os dados com valores entre 0 e 1 (Normalização min-max)
# APENAS PARA O PERFIL DE CONSUMO
# secondColumn: TRUE caso os dados tenham "time,consumption", FALSE caso tenha apenas "consumption"
normDataPerfil <- function(dados, secondColumn=TRUE){
  if(secondColumn){
    dados[,2] = (dados[,2] - min(dados[,2]))/(max(dados[,2]) - min(dados[,2]))
  } else {
    dados = (dados - min(dados))/(max(dados) - min(dados))
  }
  return(dados)
}


# Normalizar os dados baseado na média e desvio padrão (z-normalização)
# norm = (dados - media) / desvio
# O formato (shape) é igual quando normalizo min-max. O que muda são os valores no eixo y.
normDataPerfil2 <- function(dados){
  media <- mean(dados[,2])
  desvio <- sd(dados[,2])
  
  dados[,2] = (dados[,2]-media)/desvio
  return(dados)
}


# Quantidade de clusters (Elbow Method)
#     data: dados que seram agrupados
#     nc: número máximo de grupos
#     seed: um número aleatório
# (http://www.r-statistics.com/2013/08/k-means-clustering-from-r-in-action/)
wssplot <- function(data, nc=15, seed=1234){
  wss <- (nrow(data)-1)*sum(apply(data,2,var))
  for (i in 2:nc){
    set.seed(seed)
    wss[i] <- kmeans(data, centers=i, nstart = 25)$tot.withinss
    #wss[i] <- sum(kmeans(data, centers=i, nstart = 25)$withinss)
  }
  
  # Obter o joelho da curva
  elbow <- max(wss[wss <= mean(wss)])
  clusters <- which(wss == elbow)

  plot(1:nc, wss, type="b", xlab="Number of Clusters", ylab="Within groups sum of squares", main = "Elbow Method")
  abline(v=clusters, col=2, lty=2)
  return(clusters)
}


# Quantidade de clusters (Usando Silhueta)
# (https://en.wikipedia.org/wiki/Silhouette_(clustering))
# (http://stackoverflow.com/questions/15376075/cluster-analysis-in-r-determine-the-optimal-number-of-clusters/15376462#15376462)
wssplot2 <- function(data){
  pamk.best <- pamk(data, usepam = TRUE, critout = TRUE)
  # nc: the optimal number of clusters.
  cat("Number of clusters estimated by optimum average silhouette width:", pamk.best$nc, "\n")
  clusters <- pamk.best$nc
  par(mfrow = c(1,2))
  plot(pam(userData, pamk.best$nc))
  return(clusters)
}


# Ward Hierarchical Clustering
# (http://www.statmethods.net/advstats/cluster.html)
hierarchical <- function(data, cl){
  d <- dist(data, method = "euclidean") # distance matrix
  fit <- hclust(d, method="ward.D") 
  plot(fit) # display dendrogram
  groups <- cutree(fit, k=cl) # cut tree into X clusters
  # draw dendrogram with red borders around the X clusters 
  rect.hclust(fit, k=cl, border="red")
}



# Cálculo da distância euclidiana para as séries temporais
#       sqrt(sum((serie1_i - serie2_i)^2))
#
distEuclidean <- function(serie1, serie2){
  dist_euc <- 0
  for(i in 1:nrow(serie1)){
    dist_euc <- dist_euc + ((serie1[,2][i] - serie2[,2][i])^2)
  }
  
  return(sqrt(dist_euc))
}


# Correlação de pearson entre séries temporais
# Cálculo:
#   https://en.wikipedia.org/wiki/Correlation_and_dependence
#   http://stackoverflow.com/questions/3949226/calculating-pearson-correlation-and-significance-in-python
correlacao_pearson <- function(serie1, serie2){
  mean_serie1 <- mean(serie1[,2])
  mean_serie2 <- mean(serie2[,2])
  
  diffProd <- 0
  xdiff2 <- 0
  ydiff2 <- 0
  
  for(i in 1:nrow(serie1)){
    xdiff <- serie1[,2][i] - mean_serie1
    ydiff <- serie2[,2][i] - mean_serie2
    
    diffProd <- diffProd + (xdiff * ydiff)
    xdiff2 <- xdiff2 + ((xdiff)^2)
    ydiff2 <- ydiff2 + ((ydiff)^2)
  }
  
  return(diffProd / sqrt(xdiff2 * ydiff2))
}




# ----- MAE ----- #
#
# Cálculo baseado no artigo: Methods for generating TLPs (typical load profiles) for smart grid-based energy programs.
# tlp = typical load profile (predicted), rlp = real load profile (actual)
#
# Exemplo para chamar a função:
#     maeSeries(series, k.res.cor$cluster, qtdCluster.cor)
#
maeSeries <- function(series, kmeansClusters, qtdCluster){
  
  qtdObs <- nrow(series)
  qtdTotalUsers <- length(series)
  soma3 <- 0

  for(k in 1:qtdCluster){ #k
    
    # Índice de cada usuário pertencente ao cluster k
    cluster = which(kmeansClusters == k)
    # Quantidade de usuários no cluster k
    qtdUsersCluster <- length(cluster)
    
    # Curva típica (centróide) do cluster k
    predicted <- c()
    for(p in 1:qtdObs){
      predicted <- rbind( predicted, mean(t(series[p,cluster])) )
    }
    
    soma2 <- 0
    for(y in 1:qtdUsersCluster){ #nCi

      actual <- series[,cluster[y]]

      error <- actual - predicted
      m <- mean(abs(error)) # MAE
      # OU mae(actual, predicted) # do pacote "library(Metrics)"

      soma2 <- soma2 + m
    }
    
    soma3 <- soma3 + soma2
  }
  maeS <- soma3/qtdTotalUsers

  return(maeS)
}












