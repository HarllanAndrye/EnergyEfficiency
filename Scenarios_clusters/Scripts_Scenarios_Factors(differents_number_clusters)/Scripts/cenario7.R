#
# Cenário 7: Características + Perfil
#     -> Latitude, Longitude, Consumo, similaridade[]
#

source("functions.R")

# Caminho para armazenar os resultados
resultsData <- "../Results/Data/cenario7/"
...
for(numberClusters in 3:8){ # Quantidade de clusters
  ...
  cl <- kmeans(normDados, numberClusters, nstart = 1000)
  ...
  png(file = paste(resultsImages, numberClusters, "/cenario7_metricas.png", sep = ""), width = 1110, height = 611)
  par(mfrow=c(1,2))
  plot(d, type = 'l', xlab = "Execuções", ylab = "Dunn Index", main = "Cenário 7", las=1)
  plot(s, type = 'l', xlab = "Execuções", ylab = "Silhouette Width", main = "Cenário 7", las=1)
  dev.off()
  
  write(paste(numberClusters, " clusters, Dunn = ", d[qtdExecucoes], sep = ""), paste(resultsData, "dunn_cenario7.txt", sep = ""), 
        append=TRUE)
  write(paste(numberClusters, " clusters, Silhouette = ", s[qtdExecucoes], sep = ""), paste(resultsData, "silhouette_cenario7.txt", sep = ""), 
        append=TRUE)
  ...
  gg1 <- ggplot(exec1, aes(V2, V1, col=factor(cluster))) + geom_point(size = 4) + 
    labs(title="Cenário 7 - exec. 1", x="Longitude", y="Latitude") + 
  ...
  gg2 <- ggplot(exec30, aes(V2, V1, col=factor(cluster))) + geom_point(size = 4) + 
    labs(title="Cenário 7 - exec. 30", x="Longitude", y="Latitude") + 
    scale_colour_discrete(name = "Grupos") + 
    geom_text(aes(V2,V1+0.003, label=X))
  
  png(file = paste(resultsImages, numberClusters, "/cenario7_grupos.png", sep = ""), width = 1350, height = 550)
  grid.arrange(gg1, gg2, ncol = 2)
  dev.off()
  
  
  write(paste("-- Quantidade = ", numberClusters, " --", sep = ""), paste(resultsData, "grupos_cenario7.txt", sep = ""), append=TRUE)
  # Grupos - Última execução
  qtdCluster <- max(exec30$cluster)
  for(i in 1:qtdCluster){
    #print(which(exec30$cluster == i))
    # Escrevendo no arquivo...
    write(which(exec30$cluster == i), paste(resultsData, "grupos_cenario7.txt", sep = ""), append=TRUE, ncolumns = 50)
  }
  write("", paste(resultsData, "grupos_cenario7.txt", sep = ""), append=TRUE) # para pular linha
  
  
  # Perfis agrupados
  series_ <- series[-1]
  qtdCluster <- max(exec30$cluster)
  png(file = paste(resultsImages, numberClusters, "/cenario7_grupos_perfis.png", sep = ""), width = 1350, height = 550)
  ...
  write(paste(numberClusters, " clusters, MAE = ", maeSeries(series_, exec30$cluster, qtdCluster), sep = ""), 
        paste(resultsData, "mae_cenario7.txt", sep = ""), append=TRUE)
}
